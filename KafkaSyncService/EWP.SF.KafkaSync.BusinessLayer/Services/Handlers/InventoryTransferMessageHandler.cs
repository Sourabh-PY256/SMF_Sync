...
{
    public class InventoryTransferMessageHandler : ISyncMessageHandler
    {
        private readonly ILogger<InventoryTransferMessageHandler> _logger;

        public InventoryTransferMessageHandler(ILogger<InventoryTransferMessageHandler> logger)
        {
            _logger = logger;
        }

        public string[] SupportedServices => new[] { SyncERPEntity.MATERIAL_INVENTORY_TRANSFER_REQUEST_SERVICE };
        ...
        public async Task<DataSyncHttpResponse> HandleAsync(SyncMessage message, TriggerType triggerType, DataSyncServiceProcessor processor, System.IServiceProvider serviceProvider)
        {
            ...
            JObject request = JObject.Parse(message.BodyData);

            var syncData = await workOrderOperation.UpdateInventoryTransferRequest(request, message.User).ConfigureAwait(false);
