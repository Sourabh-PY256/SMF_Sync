...
public class WorkOrderRepo : IWorkOrderRepo
{
    ...
    private static async Task ProcessWorkOrderProcessesAsync(MySqlDataReader rdr, List<WorkOrder> returnValue, CancellationToken cancel)
	{
		...
		int OperationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int orderCodeOrdinal = rdr.TryGetOrdinal("OrderCode");
		int operationNoOrdinal = rdr.TryGetOrdinal("OperationNo");
		int operationTypeCodeOrdinal = rdr.TryGetOrdinal("OperationTypeCode");
		int operationSubtypeCodeOrdinal = rdr.TryGetOrdinal("OperationSubtypeCode");
		int stepOrdinal = rdr.TryGetOrdinal("Step");
		int parentCodeOrdinal = rdr.TryGetOrdinal("ParentCode");
		int machineCodeOrdinal = rdr.TryGetOrdinal("MachineCode");
		int nameOrdinal = rdr.TryGetOrdinal("Name");
		int isOutputOrdinal = rdr.TryGetOrdinal("IsOutput");
		int outputItemCodeOrdinal = rdr.TryGetOrdinal("OutputItemCode");
		int statusOrdinal = rdr.TryGetOrdinal("Status");
		int totalOrdinal = rdr.TryGetOrdinal("Total");
		int acceptedOrdinal = rdr.TryGetOrdinal("Accepted");
		int rejectedOrdinal = rdr.TryGetOrdinal("Rejected");
		int machineAcceptedOrdinal = rdr.TryGetOrdinal("MachineAccepted");
		int machineRejectedOrdinal = rdr.TryGetOrdinal("MachineRejected");
		int commentsOrdinal = rdr.TryGetOrdinal("Comments");
		int originalMachineCodeOrdinal = rdr.TryGetOrdinal("OriginalMachineCode");
		int machineStatuOrdinal = rdr.TryGetOrdinal("MachineStatus");
		int lineNoOrdinal = rdr.TryGetOrdinal("LineNo");
		int lineUIDOrdinal = rdr.TryGetOrdinal("LineUID");
		int plannedSetupStartOrdinal = rdr.TryGetOrdinal("PlannedSetupStartDate");
		int plannedSetupEndOrdinal = rdr.TryGetOrdinal("PlannedSetupEndDate");
		int plannedStartDateOrdinal = rdr.TryGetOrdinal("PlannedStartDate");
		int plannedEndDateOrdinal = rdr.TryGetOrdinal("PlannedEndDate");
		int realStartDateOrdinal = rdr.TryGetOrdinal("RealStartDate");
		int realEndDateOrdinal = rdr.TryGetOrdinal("RealEndDate");
		int realStartDateUTCOrdinal = rdr.TryGetOrdinal("RealStartDateUTC");
		int realEndDateUTCOrdinal = rdr.TryGetOrdinal("RealEndDateUTC");
		int setupTimeOrdinal = rdr.TryGetOrdinal("SetupTime");
		int execTimeOrdinal = rdr.TryGetOrdinal("ExecTime");
		int waitTimeOrdinal = rdr.TryGetOrdinal("WaitTime");
		int isBackflushOrdinal = rdr.TryGetOrdinal("IsBackflush");
		int issuedTimeOrdinal = rdr.TryGetOrdinal("IssuedTime");
		int classIdOrdinal = rdr.TryGetOrdinal("OperationClassId");
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			string id = rdr[orderCodeOrdinal].ToStr();
			WorkOrder element = returnValue.Find(x => x.Id == id);
			if (element is not null)
			{
				...
				{
					Id = rdr[OperationIdOrdinal].ToStr(),
    ...
    public bool MergeProductionOrderOperations(ProductionOrder orderInfo, string processXML, User systemOperator)
	{
		...
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_SF_ProductionOrder_Operation_MRG", connection)
				{
					CommandType = CommandType.StoredProcedure
				};
				command.Parameters.Clear();

				command.Parameters.AddCondition("_OrderCode", orderInfo.Code, !string.IsNullOrEmpty(orderInfo.Code), string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "Id"));
...
public bool MergeProductionOrderComponents(ProductionOrder orderInfo, string componentJson, User systemOperator)
	{
		bool returnValue = false;
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_SF_ProductionOrder_Op_Items_MRG", connection)
				{
					CommandType = CommandType.StoredProcedure,
   ...
   /// <summary>
	///
	/// </summary>
	public bool MergeProductionOrderByProducts(ProductionOrder orderInfo, string byProductXML, User systemOperator)
	{
		bool returnValue = false;
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_SF_ProductionOrder_ByProduct_INS", connection)
				{
					CommandType = CommandType.StoredProcedure
				};
				command.Parameters.Clear();
				...
				command.Parameters.AddCondition("_OrderCode", orderInfo.Code, !string.IsNullOrEmpty(orderInfo.Code), string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "Id"));
				command.Parameters.AddCondition("_JSON", byProductXML, !string.IsNullOrEmpty(byProductXML));
				command.Parameters.AddCondition("_Operator", () => systemOperator.Id, systemOperator is not null, string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "User"));
				command.Parameters.AddCondition("_OperatorEmployee", systemOperator.EmployeeId, !string.IsNullOrEmpty(systemOperator.EmployeeId));
...
public bool MergeProductionOrderToolingType(ProductionOrder orderInfo, string toolingJson, User systemOperator)
	{
		bool returnValue = false;
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_sf_ProductionOrder_Tooling_INS", connection)
				{
					CommandType = CommandType.StoredProcedure
				};
				command.Parameters.Clear();
	...
	///
	/// </summary>
	public bool MergeProductionOrderLabor(ProductionOrder orderInfo, string JSONData, User systemOperator)
	{
		bool returnValue = false;
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_sf_ProductionOrder_Labor_INS", connection)
				{
					CommandType = CommandType.StoredProcedure
				};
				command.Parameters.Clear();

				command.Parameters.AddCondition("_OrderCode", orderInfo.Code, !string.IsNullOrEmpty(orderInfo.Code), string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "Order Id"));
    ...
    public WorkOrderResponse MergeProductionOrder(ProductionOrder workorderInfo, User systemOperator, bool Validation, LevelMessage Level, ActionDB? mode = null, IntegrationSource intSrc = IntegrationSource.SF)
	{
		WorkOrderResponse returnValue = null;
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				...
				command.Parameters.AddCondition("_ProductId", workorderInfo.ProductId, !string.IsNullOrEmpty(workorderInfo.ProductId), string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "Product"));
				command.Parameters.AddCondition("_PlannedStart", workorderInfo.PlannedStartDate, workorderInfo.PlannedStartDate.Year > 1900);
				command.Parameters.AddCondition("_PlannedEnd", workorderInfo.PlannedEndDate, workorderInfo.PlannedEndDate.Year > 1900);
				command.Parameters.AddWithValue("_PlannedQty", workorderInfo.Quantity);
				command.Parameters.AddCondition("_RealStart", workorderInfo.ActualStartDate, workorderInfo.ActualStartDate?.Year > 1900);
				command.Parameters.AddCondition("_RealEnd", workorderInfo.ActualEndDate, workorderInfo.ActualEndDate?.Year > 1900);
				...
				{
					workorderInfo.Code = workorderInfo.OrderCode;
					workorderInfo.OrderCode = workorderInfo.OrderCode;
					returnValue.ProductionOrder = workorderInfo;
				}
			}
			catch
			{
    ...
    public async Task<ProductionOrder> GetProductionOrder(string orderCode, CancellationToken cancel = default)
	{
		ProductionOrder returnValue = null;
		...
		await using (connection.ConfigureAwait(false))
		{
			...
			await using EWP_Command command = new("SP_sf_ProductionOrder_SEL", connection)
			{
				CommandType = CommandType.StoredProcedure
			};
			...
			{
				command.Parameters.Clear();
				command.Parameters.AddCondition("_Order", orderCode, !string.IsNullOrEmpty(orderCode));
				command.Parameters.AddNull("_Status");
				command.Parameters.AddNull("_StartDate");
				command.Parameters.AddNull("_EndDate");
				command.Parameters.AddWithValue("_ListOnly", 0);
				...
				try
				{
					...
					await using (rdr.ConfigureAwait(false))
					{
						// Get ordinals for efficient access
						int orderCodeOrdinal = rdr.TryGetOrdinal("OrderCode");
						int productNameOrdinal = rdr.TryGetOrdinal("ProductName");
						int productIdOrdinal = rdr.TryGetOrdinal("ProductId");
						int plannedQtyOrdinal = rdr.TryGetOrdinal("PlannedQty");
						int plannedStartDateUTCOrdinal = rdr.TryGetOrdinal("PlannedStartDateUTC");
						int plannedStartDateOrdinal = rdr.TryGetOrdinal("PlannedStartDate");
						int plannedEndDateOrdinal = rdr.TryGetOrdinal("PlannedEndDate");
						int dueDateOrdinal = rdr.TryGetOrdinal("DueDate");
						int createDateOrdinal = rdr.TryGetOrdinal("CreateDate");
						int createUserOrdinal = rdr.TryGetOrdinal("CreateUser");
						int statusOrdinal = rdr.TryGetOrdinal("Status");
						int acceptedQtyOrdinal = rdr.TryGetOrdinal("AcceptedQty");
						int rejectedQtyOrdinal = rdr.TryGetOrdinal("RejectedQty");
						int warehouseCodeOrdinal = rdr.TryGetOrdinal("WarehouseCode");
						int productionLinesOrdinal = rdr.TryGetOrdinal("ProductionLines");
						int hasAllocationOrdinal = rdr.TryGetOrdinal("HasAllocation");
						int apsOrdinal = rdr.TryGetOrdinal("APS");
						int orderGroupOrdinal = rdr.TryGetOrdinal("OrderGroup");
						int orderTypeOrdinal = rdr.TryGetOrdinal("OrderType");
						int formulaOrdinal = rdr.TryGetOrdinal("Formula");
						int commentsOrdinal = rdr.TryGetOrdinal("Comments");
						int salesOrderOrdinal = rdr.TryGetOrdinal("SalesOrder");
						int unitOrdinal = rdr.TryGetOrdinal("Unit");
						int schedulingReadyOrdinal = rdr.TryGetOrdinal("SchedulingReady");
						int priorityOrdinal = rdr.TryGetOrdinal("Priority");
						int orderSourceOrdinal = rdr.TryGetOrdinal("OrderSource");
						int lotNoOrdinal = rdr.TryGetOrdinal("LotNo");
						int logDetailIdOrdinal = rdr.TryGetOrdinal("LogDetailId");
						int pathOrdinal = rdr.TryGetOrdinal("path");
						int updateDateOrdinal = rdr.TryGetOrdinal("UpdateDate");
						int updateUserOrdinal = rdr.TryGetOrdinal("UpdateUser");
						int realStartDateOrdinal = rdr.TryGetOrdinal("RealStartDate");
						int realStartDateUTCOrdinal = rdr.TryGetOrdinal("RealStartDateUTC");
						int realEndDateOrdinal = rdr.TryGetOrdinal("RealEndDate");
						int productCodeOrdinal = rdr.TryGetOrdinal("ProductCode");
						int versionOrdinal = rdr.TryGetOrdinal("Version");
						int lotSizeOrdinal = rdr.TryGetOrdinal("LotSize");
						// Reading main WorkOrder data
						while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
						{
							returnValue = new()
							{
								Code = rdr[orderCodeOrdinal].ToStr(),
								OrderCode = rdr[orderCodeOrdinal].ToStr(),
								Name = rdr[productNameOrdinal].ToStr(),
								ProductCode = rdr[productCodeOrdinal].ToStr(),
								ProductId = rdr[productIdOrdinal].ToStr(),
								Quantity = rdr[plannedQtyOrdinal].ToDouble(),
								PlannedStartDateUTC = rdr[plannedStartDateUTCOrdinal].ToDate(),
								PlannedStartDate = rdr[plannedStartDateOrdinal].ToDate(),
								PlannedEndDate = rdr[plannedEndDateOrdinal].ToDate(),
								DueDate = rdr[dueDateOrdinal].ToDate(),
								CreationDate = rdr[createDateOrdinal].ToDate(),
								CreatedBy = new User(rdr[createUserOrdinal].ToInt32()),
								Status = (Status)rdr[statusOrdinal].ToInt32(),
								...
								Comments = rdr[commentsOrdinal].ToStr(),
								SalesOrder = rdr[salesOrderOrdinal].ToStr(),
								UnitCode = rdr[unitOrdinal].ToStr(),
								SchedulingReady = rdr[schedulingReadyOrdinal].ToBool(),
								Priority = rdr[priorityOrdinal].ToStr(),
								OrderSource = rdr[orderSourceOrdinal].ToStr(),
								LotNo = rdr[lotNoOrdinal].ToStr(),
								LogDetailId = rdr[logDetailIdOrdinal].ToStr(),
								LotSize = rdr[lotSizeOrdinal].ToDouble(),
								Operations = []
							};
							...
							if (rdr[realStartDateOrdinal].ToDate().Year > 1900)
							{
								returnValue.ActualStartDate = rdr[realStartDateOrdinal].ToDate();
								returnValue.ActualStartDateUTC = rdr[realStartDateUTCOrdinal].ToDate();
							}
							if (rdr[realEndDateOrdinal].ToDate().Year > 1900)
							{
								returnValue.ActualEndDate = rdr[realEndDateOrdinal].ToDate();
							}
						}
						...
						if (await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderOperationsAsync(rdr, returnValue, cancel).ConfigureAwait(false);
						}

						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderMachinesAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}

						// Processing the next result set for WorkOrder components
						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderItemsAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}

						// Processing the next result set for tasks
						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderTasksAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}

						// Processing the next result set for SubProducts
						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderByproductsAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}
						...
						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderToolingValuesAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}

						// Processing the next result set for Labor
						if (returnValue is not null && await rdr.NextResultAsync(cancel).ConfigureAwait(false))
						{
							await ProcessProductionOrderLaborAsync(rdr, returnValue.Operations, cancel).ConfigureAwait(false);
						}
					}
				}
				catch (Exception ex)
    ...
    private static async Task ProcessProductionOrderOperationsAsync(MySqlDataReader rdr, ProductionOrder returnValue, CancellationToken cancel)
	{
		// Get ordinals for WorkOrder Processes
		int OperationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int OperationCodeOrdinal = rdr.TryGetOrdinal("OperationCode");
		int OperationGroupOrdinal = rdr.TryGetOrdinal("OperationGroup");
		int operationNoOrdinal = rdr.TryGetOrdinal("OperationNo");
		int operationTypeCodeOrdinal = rdr.TryGetOrdinal("OperationTypeCode");
		int operationSubtypeCodeOrdinal = rdr.TryGetOrdinal("OperationSubtypeCode");
		...
		int realStartDateUTCOrdinal = rdr.TryGetOrdinal("RealStartDateUTC");
		int realEndDateUTCOrdinal = rdr.TryGetOrdinal("RealEndDateUTC");
		ProductionOrder element = returnValue;
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			if (element is not null)
			{
				ProductionOrderOperation process = new()
				{
					OperationId = rdr[OperationIdOrdinal].ToStr(),
					OperationCode = rdr[OperationCodeOrdinal].ToStr(),
					OperationGroup = rdr[OperationGroupOrdinal].ToStr(),
					OperationTypeCode = rdr[operationTypeCodeOrdinal].ToStr(),
					OperationSubTypeCode = rdr[operationSubtypeCodeOrdinal].ToStr(),
					OperationNo = rdr[operationNoOrdinal].ToInt32(),
					Name = rdr[nameOrdinal].ToStr(),
					IsOutput = rdr[isOutputOrdinal].ToBool(),
					Status = (Status)rdr[statusOrdinal].ToInt32(),
					Quantity = rdr[totalOrdinal].ToDouble(),
					Received = rdr[acceptedOrdinal].ToDouble(),
					Rejected = rdr[rejectedOrdinal].ToDouble(),
					LineId = rdr[lineNoOrdinal].ToStr(),
					PlannedSetupStart = rdr[plannedSetupStartOrdinal].ToDate(),
					...
					PlannedStartDate = rdr[plannedStartDateOrdinal].ToDate(),
					PlannedEndDate = rdr[plannedEndDateOrdinal].ToDate(),
					ActualStartDate = rdr[realStartDateOrdinal].ToDate(),
					ActualEndDate = rdr[realEndDateOrdinal].ToDate(),
					ActualStartDateUTC = rdr[realStartDateUTCOrdinal].ToDate(),
					ActualEndDateUTC = rdr[realEndDateUTCOrdinal].ToDate(),
					Machines = [],
					Items = [],
					Byproducts = [],
					Labor = [],
				...
				if (rdr[plannedStartDateOrdinal].ToDate().Year > 1900)
				{
					process.PlannedStartDate = rdr[plannedStartDateOrdinal].ToDate();
				}
				if (rdr[plannedEndDateOrdinal].ToDate().Year > 1900)
				{
					process.PlannedEndDate = rdr[plannedEndDateOrdinal].ToDate();
				}

				if (rdr[realStartDateOrdinal].ToDate().Year > 1900)
				{
					process.ActualStartDate = rdr[realStartDateOrdinal].ToDate();
					process.ActualStartDateUTC = rdr[realStartDateUTCOrdinal].ToDate();
				}
				if (rdr[realEndDateOrdinal].ToDate().Year > 1900)
				{
					process.ActualEndDate = rdr[realEndDateOrdinal].ToDate();
					process.ActualEndDateUTC = rdr[realEndDateUTCOrdinal].ToDate();
				}
				element.Operations.Add(process);
			}
		}
	}

	private static async Task ProcessProductionOrderMachinesAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for WorkOrder Processes
		int OperationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int machineCodeOrdinal = rdr.TryGetOrdinal("MachineCode");
		int acceptedOrdinal = rdr.TryGetOrdinal("AcceptedQty");
		int rejectedOrdinal = rdr.TryGetOrdinal("RejectedQty");
		int commentsOrdinal = rdr.TryGetOrdinal("Comments");
		int originalMachineCodeOrdinal = rdr.TryGetOrdinal("OriginalMachineCode");
		int machineStatusOrdinal = rdr.TryGetOrdinal("MachineStatus");
		int lineNoOrdinal = rdr.TryGetOrdinal("LineNo");
		int lineUIDOrdinal = rdr.TryGetOrdinal("LineUID");
		int setupTimeOrdinal = rdr.TryGetOrdinal("SetupTime");
		int execTimeOrdinal = rdr.TryGetOrdinal("ExecTime");
		...
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			string id = rdr[OperationIdOrdinal].ToStr();
			ProductionOrderOperation element = returnValue.Find(x => x.OperationId == id);
			if (element is not null)
			{
				ProductionOrderMachine machine = new()
				{
					MachineCode = rdr[machineCodeOrdinal].ToStr(),
					OriginalMachineCode = rdr[originalMachineCodeOrdinal].ToStr(),
					LineId = rdr[lineNoOrdinal].ToStr(),
					LineUID = rdr[lineUIDOrdinal].ToStr(),
					Status = (Status)rdr[machineStatusOrdinal].ToInt32(),
					Received = rdr[acceptedOrdinal].ToDouble(),
					Rejected = rdr[rejectedOrdinal].ToDouble(),
					Consumption = rdr[isBackflushOrdinal].ToInt32(),
					PlannedSetupTime = rdr[setupTimeOrdinal].ToDouble(),
					PlannedExecTime = rdr[execTimeOrdinal].ToDouble(),
					WaitTime = rdr[waitTimeOrdinal].ToDouble(),
					ActualExecTime = rdr[issuedTimeOrdinal].ToDouble(),
					Labor = [],
					ToolingType = [],
				};

				element.Machines.Add(machine);
			}
		}
	}

	private static async Task ProcessProductionOrderItemsAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for WorkOrder Components
		int operationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int operationNoOrdinal = rdr.TryGetOrdinal("OperationNo");
		int itemCodeOrdinal = rdr.TryGetOrdinal("ItemCode");
		int itemNameOrdinal = rdr.TryGetOrdinal("ItemName");
		int targetQtyOrdinal = rdr.TryGetOrdinal("TargetQty");
		int targetUnitOrdinal = rdr.TryGetOrdinal("TargetUnit");
		int inputQtyOrdinal = rdr.TryGetOrdinal("InputQty");
		int inputUnitOrdinal = rdr.TryGetOrdinal("InputUnit");
		int statusOrdinal = rdr.TryGetOrdinal("Status");
		int warehouseCodeOrdinal = rdr.TryGetOrdinal("WarehouseCode");
		int commentsOrdinal = rdr.TryGetOrdinal("Comments");
		int operationTypeCodeOrdinal = rdr.TryGetOrdinal("OperationTypeCode");
		int StepOrdinal = rdr.TryGetOrdinal("Step");
		int LineNoOrdinal = rdr.TryGetOrdinal("LineNo");
		int originalItemOrdinal = rdr.TryGetOrdinal("OriginalItemCode");
		int isBackflushOrdinal = rdr.TryGetOrdinal("IsBackFlush");
		int typeOrdinal = rdr.TryGetOrdinal("Type");
		int materialImageOrdinal = rdr.TryGetOrdinal("MaterialImage");
		int lineUIDOrdinal = rdr.TryGetOrdinal("LineUID");
		int sourceOrdinal = rdr.TryGetOrdinal("Source");
		int managedByOrdinal = rdr.TryGetOrdinal("ManagedBy");
		...
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			...
			if (element is not null)
			{
				ProductionOrderItem component = new()
				{
					ItemCode = rdr[itemCodeOrdinal].ToStr(),
					OriginalItemCode = rdr[originalItemOrdinal].ToStr(),
					Quantity = rdr[targetQtyOrdinal].ToDouble(),
					UnitCode = rdr[targetUnitOrdinal].ToStr(),
					IssuedQty = rdr[inputQtyOrdinal].ToDouble(),
					Status = (Status)rdr[statusOrdinal].ToInt32(),
					WarehouseCode = rdr[warehouseCodeOrdinal].ToStr(),
					Comments = rdr[commentsOrdinal].ToStr(),
					LineId = rdr[LineNoOrdinal].ToStr(),
				...
				element.Items.Add(component);
			}
		}
	}

	private static async Task ProcessProductionOrderToolingValuesAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for ToolingType Values
		int operationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int toolingTypeCodeOrdinal = rdr.TryGetOrdinal("ToolId");
		int lineNoOrdinal = rdr.TryGetOrdinal("LineNo");
		int machineCodeOrdinal = rdr.TryGetOrdinal("MachineId");
		int quantityOrdinal = rdr.TryGetOrdinal("Quantity");
		int sourceOrdinal = rdr.TryGetOrdinal("Source");
		int commentsOrdinal = rdr.TryGetOrdinal("Comments");
		int lineUIDOrdinal = rdr.TryGetOrdinal("LineUID");
		int plannedQtyOrdinal = rdr.TryGetOrdinal("PlannedQty");
		int usageOrdinal = rdr.TryGetOrdinal("UsageCode");
		int isBackflushOrdinal = rdr.TryGetOrdinal("IsBackflush");
		int issuedTimeOrdinal = rdr.TryGetOrdinal("IssuedTime");
		...
		{
			string id = rdr[operationIdOrdinal].ToStr();
			string MachineId = rdr[machineCodeOrdinal].ToStr();
			ProductionOrderOperation element = returnValue.Find(x => x.OperationId == id);
			if (element is not null)
			{
				ProductionOrderResource toolingType = new()
				{
					Code = rdr[toolingTypeCodeOrdinal].ToStr(),
					LineId = rdr[lineNoOrdinal].ToStr(),
					Quantity = rdr[quantityOrdinal].ToDouble(),
					Source = rdr[sourceOrdinal].ToStr(),
					Comments = rdr[commentsOrdinal].ToStr(),
				...
				{
					element.Machines?.FirstOrDefault(machine => machine.LineUID == MachineId)?.ToolingType.Add(toolingType);
				}
				else
				{
					(element.ToolingType ??= []).Add(toolingType);
				}
			}
		}
	}

	private static async Task ProcessProductionOrderTasksAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for WorkOrder tasks
		int operationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int activityIdOrdinal = rdr.TryGetOrdinal("ActivityId");
		int nameOrdinal = rdr.TryGetOrdinal("Name");
		int triggerIdOrdinal = rdr.TryGetOrdinal("TriggerId");
		int sortIdOrdinal = rdr.TryGetOrdinal("SortId");
		int isMandatoryOrdinal = rdr.TryGetOrdinal("IsMandatory");
		int requiresInstructionsOrdinal = rdr.TryGetOrdinal("RequiresInstructions");
		int durationOrdinal = rdr.TryGetOrdinal("Duration");
		int durationUnitOrdinal = rdr.TryGetOrdinal("DurationUnit");
		int machineCodeOrdinal = rdr.TryGetOrdinal("MachineCode");
		int operationNoOrdinal = rdr.TryGetOrdinal("OperationNo");
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
	...
	private static async Task ProcessProductionOrderByproductsAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for SubProducts
		int operationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int itemCodeOrdinal = rdr.TryGetOrdinal("ItemCode");
		int factorOrdinal = rdr.TryGetOrdinal("Factor");
		int acceptedQtyOrdinal = rdr.TryGetOrdinal("AcceptedQty");
		int rejectedQtyOrdinal = rdr.TryGetOrdinal("RejectedQty");
		int nameOrdinal = rdr.TryGetOrdinal("Name");
		int codeOrdinal = rdr.TryGetOrdinal("Code");
		int unitTypeOrdinal = rdr.TryGetOrdinal("UnitType");
		int unitOrdinal = rdr.TryGetOrdinal("Unit");
		int warehouseCodeOrdinal = rdr.TryGetOrdinal("WarehouseCode");
		int itemImageOrdinal = rdr.TryGetOrdinal("ItemImage");
		int lineUIDOrdinal = rdr.TryGetOrdinal("LineUID");
		int lineIdOrdinal = rdr.TryGetOrdinal("LineNo");
		int commentsOrdinal = rdr.TryGetOrdinal("Comments");
		...
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			...
			if (element is not null)
			{
				ProductionOrderByProduct byProduct = new()
				{
					ItemCode = rdr[itemCodeOrdinal].ToStr(),
					Quantity = rdr[factorOrdinal].ToDouble(),
					ReceivedQty = rdr[acceptedQtyOrdinal].ToDouble(),
					RejectedQty = rdr[rejectedQtyOrdinal].ToDouble(),
					UnitCode = rdr[unitOrdinal].ToStr(),
					WarehouseCode = rdr[warehouseCodeOrdinal].ToStr(),
					LineUID = rdr[lineUIDOrdinal].ToStr(),
				...
				(element.Byproducts ??= []).Add(byProduct);
			}
		}
	}

	private static async Task ProcessProductionOrderLaborAsync(MySqlDataReader rdr, List<ProductionOrderOperation> returnValue, CancellationToken cancel)
	{
		// Get ordinals for Labor
		int operationIdOrdinal = rdr.TryGetOrdinal("OperationId");
		int positionCodeOrdinal = rdr.TryGetOrdinal("PositionCode");
		int lineNoOrdinal = rdr.TryGetOrdinal("LineNo");
		int machineCodeOrdinal = rdr.TryGetOrdinal("MachineId");
		int quantityOrdinal = rdr.TryGetOrdinal("Quantity");
		int sourceOrdinal = rdr.TryGetOrdinal("Source");
		...
		while (await rdr.ReadAsync(cancel).ConfigureAwait(false))
		{
			...
			if (element is not null)
			{
				ProductionOrderResource labor = new()
				{
					Code = rdr[positionCodeOrdinal].ToStr(),
					LineId = rdr[lineNoOrdinal].ToStr(),
					Quantity = rdr[quantityOrdinal].ToDouble(),
					Source = rdr[sourceOrdinal].ToStr(),
					Comments = rdr[commentsOrdinal].ToStr(),
				...
				if (!string.IsNullOrEmpty(MachineId))
				{
					element.Machines?.FirstOrDefault(machine => machine.LineUID == MachineId)?.Labor.Add(labor);
				}
				else
				{
					(element.Labor ??= []).Add(labor);
