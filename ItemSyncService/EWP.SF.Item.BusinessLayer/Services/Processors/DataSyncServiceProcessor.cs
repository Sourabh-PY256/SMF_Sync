...
public class DataSyncServiceProcessor
{
	...
	private async Task<DataSyncHttpResponse> SendErpDataToSf(DataSyncServiceLog LogInfo, DataSyncService ServiceData, User SystemOperator, ServiceExecOrigin ExecOrigin, string EntityCode, DataSyncHttpResponse HttpResponse, Action onResponse)
	{
		...
		if (erpResult.StatusCode == HttpStatusCode.OK)
		{
			...
			if (dataMapped is not null)
			{
				...
				{
					case BackgroundServices.ATTACHMENT_SERVICE:
					...
					case BackgroundServices.FULL_ALLOCATION_SERVICE:
						...
						{
							ResponseData sfResponse = _operations.ListUpdateAllocationBulk(listAllocStock, SystemOperator, false, LevelMessage.Success, true);
							successRecords = 0;
							if (sfResponse.IsSuccess)
							{
					...
					case BackgroundServices.DEMAND_SERVICE:
						...
						{
							List<ResponseData> sfListResponse = [];
							sfListResponse = await _operations.ListUpdateDemandBulk(listDemands, listDemandsOriginal, SystemOperator, false, LevelMessage.Success).ConfigureAwait(false);
							if (sfListResponse is not null)
							{
								...
								_ = _operations.InsertDataSyncServiceLogDetailBulk(returnDetailList);
							}
					...
					case BackgroundServices.CLOCKINOUT_SERVICE:
						...
						double clockInOffset = _operations.GetTimezoneOffset("ERP") * -1;
						...
						{
							List<ResponseData> sfListResponse;
							sfListResponse = _operations.ListUpdateCLockInOutBulk(listClockInRecords, listClockInRecordsOriginal, SystemOperator, false, LevelMessage.Success);
							if (sfListResponse is not null)
							{
								...
								_ = _operations.InsertDataSyncServiceLogDetailBulk(returnClockList);
							}
							LogInfo.SfResponseJson = JsonConvert.SerializeObject(new { SuccessRecords = successRecords, FailedRecords = failedRecords, Data = sfListResponse.Select(x => new { x.Code, x.IsSuccess, x.Message }) });
						}
						break;
					...
					case BackgroundServices.MATERIAL_RETURN_SERVICE:
						...
						if (listMaterialReturns.Count > 0)
						{
							...
							foreach (MaterialReturnExternal elem in listMaterialReturns)
							{
								...
								try
								{
									sfResponse = (await _operations.ListUpdateMaterialReturn(
											listElem,
											SystemOperator,
											false,
											LevelMessage.Success
					...
					case BackgroundServices.PRODUCT_RETURN_SERVICE:
						...
						if (listProductReturns.Count > 0)
						{
							...
							foreach (ProductReturnExternal elem in listProductReturns)
							{
								...
								try
								{
									sfResponse = (await _operations.ListUpdateProductReturn(
									listElem,
									SystemOperator,
									false,
									LevelMessage.Success
					...
					case BackgroundServices.FULL_STOCK_SERVICE:
						List<StockExternal> listFstock = JsonConvert.DeserializeObject<List<StockExternal>>(dataJson);
						LogInfo.SfMappedJson = JsonConvert.SerializeObject(listFstock);
						LogInfo.SfProcessDate = DataSyncServiceUtil.ConvertDate(ServiceData.ErpData.DateTimeFormat, DateTime.Now, ServiceData.ErpData.TimeZone);
						_ = await _operations.InsertDataSyncServiceLog(LogInfo).ConfigureAwait(false);
						if (listFstock.Count > 0)
						{
							List<ResponseData> sfListResponse = [];
							ResponseData sfResponse = _operations.ListUpdateStockBulk(listFstock, SystemOperator, false, LevelMessage.Success);
							successRecords = 0;
							if (sfResponse.IsSuccess)
							{
								successRecords = sfResponse.Code.ToInt32();
							}
							failedRecords = listFstock.Count - successRecords;
							LogInfo.SfResponseJson = JsonConvert.SerializeObject(sfResponse);
						}
						break;

					case BackgroundServices.SUPPLY_SERVICE:
						List<SupplyExternal> listSupplies = JsonConvert.DeserializeObject<List<SupplyExternal>>(dataJson);
						List<SupplyExternal> listSuppliesOriginal = JsonConvert.DeserializeObject<List<SupplyExternal>>(dataJsonOriginal);
						LogInfo.SfMappedJson = JsonConvert.SerializeObject(listSupplies);
						LogInfo.SfProcessDate = DataSyncServiceUtil.ConvertDate(ServiceData.ErpData.DateTimeFormat, DateTime.Now, ServiceData.ErpData.TimeZone);
						_ = await _operations.InsertDataSyncServiceLog(LogInfo).ConfigureAwait(false);
						List<DataSyncServiceLogDetail> returnDataSplList = [];
						if (listSupplies.Count > 0)
						{
							List<ResponseData> sfListResponse = [];
							sfListResponse = await _operations.ListUpdateSupplyBulk(listSupplies, listSuppliesOriginal, SystemOperator, false, LevelMessage.Success).ConfigureAwait(false);
							if (sfListResponse is not null)
							{
								sfListResponse.ForEach(rsp =>
								{
									...
									try
									{
										...
										returnDataSplList.Add(LogSingleInfo);
									}
									catch (Exception ex)
									{
										failedRecords++;
								...
								_ = _operations.InsertDataSyncServiceLogDetailBulk(returnDataSplList);
							}
							LogInfo.SfResponseJson = JsonConvert.SerializeObject(new { SuccessRecords = successRecords, FailedRecords = failedRecords, Data = sfListResponse.Select(x => new { x.Code, x.IsSuccess, x.Message }) });
						}
						break;
					...
					case BackgroundServices.PROCESS_TYPE_SERVICE:
						...
						{
							List<ResponseData> sfListResponse = [];
							sfListResponse = _operations.ListUpdateSuboperationTypes_Bulk(listProcessTypes, SystemOperator, false, LevelMessage.Success);
							if (sfListResponse is not null)
							{
								sfListResponse.ForEach(rsp =>
								{
								...
								_ = _operations.InsertDataSyncServiceLogDetailBulk(returnDetailListProcess);
							}
							LogInfo.SfResponseJson = JsonConvert.SerializeObject(new { SuccessRecords = successRecords, FailedRecords = failedRecords, Data = sfListResponse.Select(x => new { x.Code, x.IsSuccess, x.Message }) });
						}
					...
					case BackgroundServices.PRODUCT_TRANSFER_SERVICE:
						...
						if (listProductTransfers.Count > 0)
						{
							...
							foreach (ProductTransferExternal elem in listProductTransfers)
							{
								...
								try
								{
									sfResponse = (await _operations.ListUpdateProductTransfer(
									listElem,
									SystemOperator,
									false,
									LevelMessage.Success
					...
					case BackgroundServices.MATERIAL_SCRAP_SERVICE:
						...
						if (listMaterialScrapIssues.Count > 0)
						{
							...
							foreach (MaterialIssueExternal elem in listMaterialScrapIssues)
							{
								...
								try
								{
									sfResponse = (await _operations.ListUpdateMaterialScrap(
									listElem,
									SystemOperator,
									false,
									LevelMessage.Success
					...
					case BackgroundServices.PROCEDURE_SERVICE:
						...
						if (listProcedures.Count > 0)
						{
							...
							foreach (ProcedureExternalSync elem in listProcedures)
							{
								...
								try
								{
									sfResponse = (await _operations.ProcessMasterInsExternalSync(
									listElem,
					...
					case BackgroundServices.MACHINE_ISSUE_SERVICE:
						...
						break;
					case BackgroundServices.LABOR_ISSUE_SERVICE:
	...
	private static async Task<DataSyncHttpResponse> SendSfDataToErp(DataSyncServiceLog LogInfo, DataSyncService ServiceData, User SystemOperator, ServiceExecOrigin ExecOrigin, string RequestBody, DataSyncHttpResponse HttpResponse, Action onResponse)
	{
	...
	private static async Task ReprocessError(string ErpJson, string DataJson, DataSyncService ServiceData, DataSyncServiceLog LogInfo, DataSyncServiceLogDetail LogDetail, User SystemOperator)
	{
		...
		switch (ServiceData.Entity.Name)
		{
			case BackgroundServices.ALLOCATION_SERVICE:
			case BackgroundServices.FULL_ALLOCATION_SERVICE:
				StockAllocationExternal stockAllocation = JsonConvert.DeserializeObject<StockAllocationExternal>(DataJson);
				if (stockAllocation is not null)
				{
					StockAllocationExternal[] listElem = [stockAllocation];
					ResponseData sfResponse = _operations.ListUpdateAllocationBulk(
						listElem,
						SystemOperator,
						false,
						LevelMessage.Success, true
					);
			...
			case BackgroundServices.WORKCENTER_SERVICE:
			case BackgroundServices.PRODUCTION_LINE_SERVICE:
				AssetExternal asset = JsonConvert.DeserializeObject<AssetExternal>(DataJson);
				if (asset is not null)
				{
					List<AssetExternal> listElem = [asset];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.BIN_LOCATION_SERVICE:
				BinLocationExternal binLocation = JsonConvert.DeserializeObject<BinLocationExternal>(DataJson);
				if (binLocation is not null)
				{
					List<BinLocationExternal> listElem = [binLocation];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.DEMAND_SERVICE:
				DemandExternal demand = JsonConvert.DeserializeObject<DemandExternal>(DataJson);
				if (demand is not null)
				{
					List<DemandExternal> listElem = [demand];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
					...
					try
					{
						sfResponse = (await _operations.ListUpdateDemand(
						listElem,
						null,
						SystemOperator,
						false,
			...
			case BackgroundServices.EMPLOYEE_SERVICE:
				EmployeeExternal employee = JsonConvert.DeserializeObject<EmployeeExternal>(DataJson);
				if (employee is not null)
				{
					List<EmployeeExternal> listElem = [employee];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.INVENTORY_SERVICE:
				InventoryExternal inventory = JsonConvert.DeserializeObject<InventoryExternal>(DataJson);
				if (inventory is not null)
				{
					List<InventoryExternal> listElem = [inventory];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.INVENTORY_STATUS_SERVICE:
				InventoryStatusExternal inventoryStatus = JsonConvert.DeserializeObject<InventoryStatusExternal>(DataJson);
				if (inventoryStatus is not null)
				{
					List<InventoryStatusExternal> listElem = [inventoryStatus];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.ITEM_SERVICE:
				ComponentExternal item = JsonConvert.DeserializeObject<ComponentExternal>(DataJson);
				if (item is not null)
				{
					List<ComponentExternal> listElem = [item];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
					...
					try
					{
						sfResponse = (await _operations.ListUpdateComponent(
						listElem,
						SystemOperator,
						false,
						LevelMessage.Success
			...
			case BackgroundServices.CLOCKINOUT_SERVICE:
				{
					List<ClockInOutDetailsExternal> listClockinRecords = [];
					ClockInOutDetailsExternal clockDetail = JsonConvert.DeserializeObject<ClockInOutDetailsExternal>(DataJson);
					if (clockDetail is not null)
					{
						listClockinRecords.Add(clockDetail);
					}
					double clockinOffset = _operations.GetTimezoneOffset("ERP") * -1;
					listClockinRecords.ForEach(itm =>
					{
						if (itm.StartDate.HasValue && itm.StartDate.Value.Year <= 1900)
						{
							itm.StartDate = null;
					...
					if (listClockinRecords.Count > 0)
					{
						...
						try
						{
							sfResponse = _operations.ListUpdateCLockInOutBulk(listClockinRecords, null, SystemOperator, false, LevelMessage.Success).FirstOrDefault();
						}
						catch (Exception ex)
						{
							sfResponse = new ResponseData
			...
			case BackgroundServices.LOT_SERIAL_STATUS_SERVICE:
				LotSerialStatusExternal lotSerialStatus = JsonConvert.DeserializeObject<LotSerialStatusExternal>(DataJson);
				if (lotSerialStatus is not null)
				{
					List<LotSerialStatusExternal> listElem = [lotSerialStatus];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.MACHINE_SERVICE:
				MachineExternal machine = JsonConvert.DeserializeObject<MachineExternal>(DataJson);
				if (machine is not null)
				{
					List<MachineExternal> listElem = [machine];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.MATERIAL_ISSUE_SERVICE:
				MaterialIssueExternal materialIssue = JsonConvert.DeserializeObject<MaterialIssueExternal>(DataJson);
				if (materialIssue is not null)
				{
					List<MaterialIssueExternal> listElem = [materialIssue];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.PRODUCT_SERVICE:
				ProductExternal product = JsonConvert.DeserializeObject<ProductExternal>(DataJson);
				if (product is not null)
				{
					List<ProductExternal> listElem = [product];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.PRODUCTION_ORDER_CHANGE_STATUS_SERVICE:
				ProductionOrderChangeStatusExternal productionOrderChangeStatus = JsonConvert.DeserializeObject<ProductionOrderChangeStatusExternal>(DataJson);
				if (productionOrderChangeStatus is not null)
				{
					List<ProductionOrderChangeStatusExternal> listElem = [productionOrderChangeStatus];
					WorkOrderResponse sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.PRODUCTION_ORDER_SERVICE:
				WorkOrderExternal workOrder = JsonConvert.DeserializeObject<WorkOrderExternal>(DataJson);
				if (workOrder is not null)
				{
					List<WorkOrderExternal> listElem = [workOrder];
					double offset = _operations.GetTimezoneOffset("ERP") * -1;
					Operations.AddWorkOrderDatesOffset(workOrder, offset);
			...
			case BackgroundServices.PRODUCT_RECEIPT_SERVICE:
				ProductReceiptExternal productReceipt = JsonConvert.DeserializeObject<ProductReceiptExternal>(DataJson);
				if (productReceipt is not null)
				{
					List<ProductReceiptExternal> listElem = [productReceipt];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.PRODUCT_RETURN_SERVICE:
				ProductReturnExternal productReturn = JsonConvert.DeserializeObject<ProductReturnExternal>(DataJson);
				if (productReturn is not null)
				{
					List<ProductReturnExternal> listElem = [productReturn];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
			...
			case BackgroundServices.WAREHOUSE_SERVICE:
				WarehouseExternal warehouse = JsonConvert.DeserializeObject<WarehouseExternal>(DataJson);
				if (warehouse is not null)
				{
					List<WarehouseExternal> listElem = [warehouse];
					ResponseData sfResponse = null;
					DataSyncServiceLogDetail LogSingleInfo = new()
					{
						Id = LogDetail.Id,
	...
	private static async Task SetRequestHeaders(DataSyncService ServiceData, APIWebClient client)
	{
		// Required Headers
	...
	private static async Task<DataSyncResponse> ErpSendRequestAsync(DataSyncService ServiceData, ServiceExecOrigin ExecOrigin, string BodyData = "", bool triggerRevalidate = false, DataSyncServiceLog LogInfo = null)
	{
		string httpMethod = string.Empty;
		if (!string.IsNullOrEmpty(ServiceData.HttpMethod))
		{
	...
	private static async Task<DataSyncErpAuth> GetErpToken(APIWebClient client, DataSyncService ServiceData)
	{
	...
	private static void ErpTokenRenewal(DataSyncService ServiceData, HttpResponseHeaders ResponseHeaders)
	{
