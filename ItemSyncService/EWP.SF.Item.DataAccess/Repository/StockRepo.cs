...
public class StockRepo : IStockRepo
{
    ...
    public StockRepo(IApplicationSettings applicationSettings)
    {
	...
	///
	/// </summary>
	public ResponseData MergeStock_Bulk(string request)
	{
		...
		using (EWP_Connection connection = new(ConnectionString))
		{
			try
			{
				using EWP_Command command = new("SP_SF_Stock_BLK", connection)
				{
					CommandType = CommandType.StoredProcedure
				};
				command.Parameters.Clear();
				command.Parameters.AddCondition("_JSON", request, !string.IsNullOrEmpty(request), string.Format(CultureInfo.InvariantCulture, MISSING_PARAM, "Stocks"));
				command.Parameters.AddNull("_SyncType");
